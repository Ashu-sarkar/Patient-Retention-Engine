...
- Lets doctors sign in with onboarding-created usernames/passwords and issue prescription PDFs from a personal dashboard
...
Configure and deploy `doctor-dashboard/index.html`, then share the deployed `doctor-dashboard/` URL with doctors. Doctors log in with the username and password created during hospital onboarding; the dashboard resolves the Supabase Auth user to the matching `doctor_profiles` row.
