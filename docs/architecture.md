...
- **Doctor app:** `doctor-dashboard/index.html` is a Supabase Auth dashboard for queue review, prescription drafting, PDF issue, and delivery handoff. Doctors authenticate with the username/password created during hospital onboarding; Supabase RLS scopes data to the claimed doctor profile. Prescription delivery goes through the `prescription-delivery` Supabase Edge Function, which verifies the doctor session and signs the n8n WF13 call with an internal HMAC secret. Patient PDF links go through the `prescription-pdf` gateway, which validates an HMAC token and creates a fresh Supabase Storage signed URL from `pdf_storage_path`.
