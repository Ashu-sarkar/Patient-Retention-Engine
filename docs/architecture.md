...
- **Doctor app:** `doctor-dashboard/index.html` is a Supabase Auth dashboard for queue review, prescription drafting, PDF issue, and delivery handoff. Doctors authenticate with WhatsApp OTP using the registered onboarding phone; Supabase RLS scopes data to the claimed doctor profile. Prescription delivery goes through the `prescription-delivery` Supabase Edge Function, which verifies the doctor session and signs the n8n WF13 call with an internal HMAC secret.
