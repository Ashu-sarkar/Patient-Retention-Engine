...
while IFS= read -r line || [[ -n "${line}" ]]; do
  ...
  [[ "${line}" == *"="* ]] || continue
  case "${line%%=*}" in
    N8N_PRESCRIPTION_DELIVERY_URL|INTERNAL_WEBHOOK_SECRET|DOCTOR_DASHBOARD_ORIGIN|SUPABASE_URL|TWILIO_ACCOUNT_SID|TWILIO_AUTH_TOKEN|TWILIO_WHATSAPP_FROM|TWILIO_CONTENT_PRESCRIPTION_DELIVERY|TWILIO_CONTENT_PRESCRIPTION_WITH_FOLLOWUP|TWILIO_CONTENT_PRESCRIPTION_JOURNEY_START|TWILIO_STATUS_CALLBACK_URL)
      export "${line%%=*}=${line#*=}"
      ;;
  esac
done < .env
...
npx supabase secrets set --project-ref "${PROJECT_REF}" \
  ...
  "TWILIO_CONTENT_PRESCRIPTION_WITH_FOLLOWUP=${TWILIO_CONTENT_PRESCRIPTION_WITH_FOLLOWUP:-}" \
  "TWILIO_CONTENT_PRESCRIPTION_JOURNEY_START=${TWILIO_CONTENT_PRESCRIPTION_JOURNEY_START:-}" \
